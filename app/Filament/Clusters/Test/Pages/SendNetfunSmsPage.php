...
>>>>>>> 9b82f1c (.)
    ...
    public function getSmsFormSchema(): array
    {
        return [
            'to' => Forms\Components\TextInput::make('to')
                ->label((string) __('notify::sms.form.to.label'))
                ->tel()
                ->required()
                ->helperText((string) __('notify::sms.form.to.helper'))
                ->placeholder('+393401234567'),
            'from' => Forms\Components\TextInput::make('from')
                ->label((string) __('notify::sms.form.from.label'))
                ->default(config('app.name', 'Our Platform'))
                ->required()
                ->maxLength(11)
                ->helperText((string) __('notify::sms.form.from.helper')),
            'body' => Forms\Components\Textarea::make('body')
                ->label((string) __('notify::sms.form.body.label'))
                ->required()
                ->rows(3)
                ->helperText((string) __('notify::sms.form.body.helper')),
            'provider' => Forms\Components\Select::make('provider')
                ->label((string) __('notify::sms.form.provider.label'))
                ->options([
                    'netfun' => 'Netfun',
        ...
        */
        ...
        try {
            ...
            FilamentNotification::make()
                ->success()
                ->title((string) __('notify::sms.notifications.sent.title'))
                ->body((string) __('notify::sms.notifications.sent.body'))
                ->send();
            ...
            FilamentNotification::make()
                ->danger()
                ->title((string) __('notify::sms.notifications.error.title'))
                ->body($e->getMessage())
                ->send();
        }
    }
    ...
    protected function getSmsFormActions(): array
    {
        return [
            Action::make('sendSms')
                ->label((string) __('notify::sms.actions.send'))
                ->submit('sendSms'),
        ];
    }
