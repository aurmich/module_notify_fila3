...
@section('content')
        ...
        <tr>
            <td style="text-align: center;">
                <h1 style="color: #2D3748; font-size: 24px; margin: 0 0 20px 0;">
                    {{ (string) __('notify::mail.order.confirmation_title') }}
                </h1>
                <p style="color: #4A5568; font-size: 16px; margin: 0 0 30px 0;">
                    {{ (string) __('notify::mail.order.confirmation_message', ['order_id' => $order->id]) }}
                </p>
            </td>
        </tr>
    </table>
        ...
        <tr>
            <td style="padding: 20px;">
                <h2 style="color: #2D3748; font-size: 18px; margin: 0 0 15px 0;">
                    {{ (string) __('notify::mail.order.details') }}
                </h2>

                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                        <td style="padding: 10px 0; border-bottom: 1px solid #E2E8F0;">
                            <span style="color: #4A5568;">{{ (string) __('notify::mail.order.order_number') }}:</span>
                            <span style="color: #2D3748; font-weight: bold;">{{ $order->id }}</span>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 10px 0; border-bottom: 1px solid #E2E8F0;">
                            <span style="color: #4A5568;">{{ (string) __('notify::mail.order.date') }}:</span>
                            <span style="color: #2D3748;">{{ $order->created_at->format('d/m/Y H:i') }}</span>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding: 10px 0; border-bottom: 1px solid #E2E8F0;">
                            <span style="color: #4A5568;">{{ (string) __('notify::mail.order.total') }}:</span>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 30px;">
        <tr>
            <td>
                <h2 style="color: #2D3748; font-size: 18px; margin: 0 0 15px 0;">
                    {{ (string) __('notify::mail.order.items') }}
                </h2>
            </td>
        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <p style="color: #4A5568; font-size: 14px; margin: 0;">
                                    {{ (string) __('notify::mail.order.quantity') }}: {{ $item->quantity }}
                                </p>
                            </td>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom: 30px;">
        <tr>
            <td>
                <h2 style="color: #2D3748; font-size: 18px; margin: 0 0 15px 0;">
                    {{ (string) __('notify::mail.order.shipping_address') }}
                </h2>
            ...
            <tr>
                <td style="text-align: center;">
                    <a href="{{ $order->tracking_url }}" class="button" style="background-color: #4299E1; color: white; padding: 12px 24px; border-radius: 6px; text-decoration: none; font-weight: bold; display: inline-block;">
                        {{ (string) __('notify::mail.order.track_order') }}
                    </a>
                </td>
            </tr>
        </table>
        ...
        <tr>
            <td style="text-align: center; padding-top: 30px; border-top: 1px solid #E2E8F0;">
                <p style="color: #718096; font-size: 14px; margin: 0 0 10px 0;">
                    {{ (string) __('notify::mail.order.questions') }}
                </p>
                <a href="{{ config('notify.support_url') }}" style="color: #4299E1; text-decoration: none;">
                    {{ (string) __('notify::mail.order.contact_support') }}
                </a>
            </td>
        </tr>
    </table>
